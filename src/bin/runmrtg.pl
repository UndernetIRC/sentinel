...
<html>
  <head>
    ...
    <title>Sentinel bot graphics</title>
  </head>
  <body bgcolor=#F0ECEB>
  <font face=Verdana>
  <h2>Sentinel</h2><br><br>
<b>LOCAL USERS</b><br>
...
open(LOG,"$path/var/sentinel.log");
while(<LOG>)
{
	chop;
	my ($name,$value)=split(/:/);
...
sub syntax
{
	print STDERR "syntax : $0 /path/to/mrtg /path/to/sentinel/configfile.conf\n";
	print STDERR "example: $0 /usr/local/bin/mrtg /home/sentinel/etc/sentinel.conf\n";
	exit 1;
}
